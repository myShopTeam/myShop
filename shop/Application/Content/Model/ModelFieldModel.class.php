        ...
        if ($data) {
            ...
            if ($this->addFieldSql($field_type, $field)) {
                ...
                if ($fieldid) {
                    ...
                    $this->error = '字段信息入库失败！';
                    //回滚
                    $this->query("ALTER TABLE  `{$field['tablename']}` DROP  `{$field['fieldname']}`");
                    return false;
                }
            } else {
    ...
    protected function editFieldSql($field_type, $field) {
        ...
        switch ($field_type) {
            ...
            case 'int':
                ...
                break;
            case 'mediumtext':
                $sql = "ALTER TABLE `{$tablename}` CHANGE `{$oldfilename}` `{$newfilename}` MEDIUMTEXT NOT NULL";
                if (false === $this->query($sql)) {
                    $this->error = '字段结构更改失败！';
                    return false;
                }
                break;
            case 'text':
                $sql = "ALTER TABLE `{$tablename}` CHANGE `{$oldfilename}` `{$newfilename}` TEXT NOT NULL";
                if (false === $this->query($sql)) {
                    $this->error = '字段结构更改失败！';
                    return false;
                }
                break;
            case 'date':
                $sql = "ALTER TABLE `{$tablename}` CHANGE `{$oldfilename}` `{$newfilename}` DATE NULL";
                if (false === $this->query($sql)) {
                    $this->error = '字段结构更改失败！';
                    return false;
                }
                break;
            case 'datetime':
                $sql = "ALTER TABLE `{$tablename}` CHANGE `{$oldfilename}` `{$newfilename}` DATETIME NULL";
                if (false === $this->query($sql)) {
                    $this->error = '字段结构更改失败！';
                    return false;
                }
                break;
            case 'timestamp':
                $sql = "ALTER TABLE `{$tablename}` CHANGE `{$oldfilename}` `{$newfilename}` TIMESTAMP NOT NULL";
                if (false === $this->query($sql)) {
                    $this->error = '字段结构更改失败！';
                    return false;
                }
            ...
            case "bigint":
                ...
                break;
            case "longtext":
                $sql = "ALTER TABLE `{$tablename}` CHANGE `{$oldfilename}` `{$newfilename}`  LONGTEXT NOT NULL";
                if (false === $this->query($sql)) {
                    $this->error = '字段结构更改失败！';
                    return false;
                }
