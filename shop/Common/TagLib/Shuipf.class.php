...
class Shuipf extends TagLib {
    ...
    public function _navigate($tag, $content) {
        ...
        if (is_numeric($catid)) {
            ...
            $parsestr .= 'if(getCategory($cid,\'menuurl\')!=\'\'){';
			$parsestr .= '      $parsestr[] = \'<a href="\' . getCategory($cid,\'menuurl\')  . \'" ' . $target . '>\' . getCategory($cid,\'catname\') . \'</a>\';';
            $parsestr .= '}else{';
     ...
     * 标签：<get></get>
     * 作用：特殊标签，SQL查询标签
     * 用法示例：<get sql="SELECT * FROM demo_article  WHERE status=99 ORDER BY inputtime DESC" page="$page" num="5"> .. HTML ..</get>
     * 参数说明：
